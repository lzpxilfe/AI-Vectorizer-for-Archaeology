...
> 선에 커서를 둔 뒤 `G`를 누르세요 (`⤴ 라벨 공백 연결`, 실험적). 반경 3px 내에서
> 가까운 Ink 방향을 읽어 보조 강도가 적용된 곡선을 보여 줍니다. 미리보기 끝점 클릭으로
> 채택하고 `Esc` 또는 다른 위치 클릭으로 취소합니다. 커서 이동은 취소가 아닙니다.
> 방향이 충돌하거나 공백 방향과 맞지 않는 경우, 3–128px 범위를 벗어난 경우에는 기존
> Ink를 유지합니다. 합성 예시 검증 단계이며 지도에서 직접 선을 검수해야 합니다.
