...
- 🟧 Ink 추적 중 `Alt`+두 번 클릭으로 문자·기호·얼룩을 부드럽게 피하는
  session-only 회피 영역
- 🪜 숫자 라벨 때문에 실제로 끊긴 검은·갈색 등고선을 사용자가 검수해 이어 주는
  `⤴ 라벨 공백 연결` preview
...
검은·갈색 등고선이 숫자 라벨 때문에 **실제로 끊긴** 경우에만 쓰는, 자동 연결이 아닌
사용자 확인형 기능입니다.

```text
공백 전 anchor 확정 → 반대편 등고선에 hover → G → 초록 preview 검토 → 같은 끝점 클릭
```

| 상황 | 동작 |
| --- | --- |
| 양쪽에 공백 바깥쪽 Ink 지지가 있고 방향이 일치 | assist 강도를 반영한 제한된 곡선 preview |
| 글자 획이 3px 방향장을 가림 | 두 명시적 endpoint의 바깥쪽 12px 중심선만 다시 확인 |
| 대상선이 없거나 평행선·문자만 보임, 방향이 퍼짐 | **연결하지 않고 기존 Ink preview 유지** |

끝점을 클릭해야 채택하며 `Esc` 또는 다른 위치 클릭은 proposal만 취소합니다. hover는
미리보기를 지우지 않습니다. OCR·model·자동 endpoint 선택은 사용하지 않습니다.
합성 제품 경로에서 재현·검증했지만, 실제 고지도 정확도나 사용자 작업 시간의 근거는
아닙니다.
