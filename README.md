...
확정하고, 반대편 선 위에서 `G`를 누르면 실험적 라벨 공백 연결을 요청할 수 있습니다.
가까운 Ink 중심선의 방향으로 만든 곡선에 보조 강도를 적용해 미리 보여 줍니다.
끝점을 클릭하면 채택하고, `Esc` 또는 다른 위치 클릭으로 취소합니다. 커서 이동 중에도
미리보기는 유지됩니다. 3px 안의 방향이 글자 때문에 충돌하면, 두 명시적 endpoint에서
공백 반대쪽으로 이어지는 최대 12px의 중심선 지지가 충분할 때에만 다시 방향을 읽습니다.
그 지지가 부족하거나 선 방향이 공백과 맞지 않으면 기존 Ink를 유지합니다. 이 기능의
검증은 합성 예시 단계입니다.
